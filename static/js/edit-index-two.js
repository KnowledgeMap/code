var buttonEvent = (function (button){
	...
	svgEvent.modeling = function (obj){
		...
		var text = nodes.append("image")
		  ...
		  var text = nodes.append('rect')
		  				  .attr("class",'rect')
		  				  .attr('width',50)
		  				  .attr('height',50)
		  				  .attr('fill',function (data){var colors =  data.group ? color(data.group) : "#aaa"; svgEvent.tempData = data; return colors;});

		  var rect = nodes.append("image")
		      .attr("xlink:href", function (data){ var src = data.group ? "" : "https://github.com/favicon.ico"; return src})
		      .attr("x", 0)
		      .attr("y", 0)
		      .attr("width", 16)
		      .attr("height", 16);

		  	var data =   nodes.append("text")
		      .attr('class','text')
		      .text(function(d) { var name = d.group ? d.name : ""; return name});
		...
		force.on("tick", function() {
							   ...
							   /* data	.attr("x", function(data) { return data.x; })
										.attr("y", function(data) { return data.y; });*/
							});
	}
